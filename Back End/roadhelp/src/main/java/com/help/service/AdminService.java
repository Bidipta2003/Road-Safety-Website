...
public class AdminService {
    ...
    public ServiceResponse<Boolean> updateWorkCompleted(List<MultipartFile> images, int postId) {
        ...
        post.get().setAfterWorkImagePath1(postImagePaths[0].replace("D:\\allMedia",""));
        post.get().setAfterWorkImagePath2(postImagePaths[1].replace("D:\\allMedia",""));
        post.get().setAfterWorkImagePath3(postImagePaths[2].replace("D:\\allMedia",""));
        post.get().setAfterWorkImagePath4(postImagePaths[3].replace("D:\\allMedia",""));
        post.get().setAfterWorkImagePath5(postImagePaths[4].replace("D:\\allMedia",""));
    ...
    private String[] savePostAfterWorkImages(List<MultipartFile> images, String root){
        ...
        try{
            for(MultipartFile image:images){
                ...
                Path postImagePath= Paths.get("D:/allMedia/afterWork");
    ...
    private void deleteAdminProfileImage(String image){
        String root=Paths.get("").toAbsolutePath().toString();
        try{Files.delete(Paths.get("D:/allMedia/adminProfileImage"));}catch (Exception e){e.printStackTrace();}
    }
}
